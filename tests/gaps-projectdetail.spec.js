...
test.describe('项目详情 成员管理', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    let members = [{ id: 'u_001', name: '管理员', email: 'admin@lambs.local', role: 'super_admin' }];
    let nonMembers = [
      { id: 'u_002', name: '张三', email: 'zhangsan@lambs.local', role: 'project_admin' },
      { id: 'u_003', name: '李四', email: 'lisi@lambs.local', role: 'viewer' },
    ];
    await page.route(/\/api\/projects\/qa-tools-hub\/members/, (route) => {
  ...
  test('成员管理：列表渲染（已分配 + 可添加）', async ({ page }) => {
    ...
    await expect(page.getByText('admin@lambs.local').first()).toBeVisible();
    await expect(page.getByText('可添加用户')).toBeVisible();
    await expect(page.getByText('zhangsan@lambs.local')).toBeVisible();
  });
...
test.describe('项目详情 服务日志', () => {
  test('服务日志：列表渲染 + 刷新重新请求', async ({ page }) => {
    ...
    let logCalls = 0;
    await page.route(/\/api\/projects\/qa-tools-hub\/logs$/, (route) => {
      logCalls++;
...
test.describe('项目详情 备份管理', () => {
  const withBackups = async (page, initialBackups, captures = {}) => {
    ...
    await page.route('**/api/backups/qa-tools-hub/download/*', (route) => {
  ...
  test('备份管理：列表渲染 + 创建备份', async ({ page }) => {
    ...
    expect(captures.postUrl).toContain('/api/backups/qa-tools-hub');
